...
function preload() {
	img1=loadImage('images/1y3.png');
	img2=loadImage('images/2.png');
	img3=loadImage('images/4.png');
	img4=loadImage('images/5.png');
	img5=loadImage('images/6.png');
	img6=loadImage('images/7.png');
}
...
function draw() {
  ...
  mov=mov+2;
	...
	if(frameCount>0 && frameCount<250){
		image(img1,holi+24,holi);
  	if(holi<180){
  		holi=holi+1;
  	}
	}

	if(frameCount>=250 && frameCount<290){
		image(img2,208,205);
	}

	if(frameCount>=290 && frameCount<310){
		image(img1,holi+24,holi);
	}

	if(frameCount>=310 && frameCount<330){
		image(img3,holi+24,holi);
	}

	if(frameCount>=330 && frameCount<350){
		image(img4,holi+24,holi);
	}

	if(frameCount>=350 && frameCount<385){
		image(img5,holi+24,holi);
		holi=holi+2;
	}

	if(frameCount>=385){
		image(img6,holi+24,holi);
	}
